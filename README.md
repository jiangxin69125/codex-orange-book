...
| /stop | 停止所有后台终端任务 | 一次终止当前会话的全部后台命令 | 已确认所有后台任务都可以安全中断 |
...
`/stop` 不能选择单个任务，它会停止当前会话里的**所有**后台终端。
如果还有数据库迁移、文件写入、部署等不能中断的任务，不要使用 `/stop`；只需要停一个任务时，先用 `/ps` 确认目标，再明确要求 Codex 只终止那一个命令。
...
| 想停止所有后台命令 | 已确认每个后台任务都可安全中断 | 用 /ps 检查后再用 /stop |
...
> **安全警告：下面的网站和案例只用于静态前端演示。**
> 演示里的注册信息、密码、收货人、电话、地址和订单都保存在当前浏览器的本地存储中；它没有真正的服务端认证、密码加密或后台权限控制。
> 不要输入真实邮箱、在其他网站使用过的密码、真实电话或地址，也不要把它当作真实商店上线。
> 生产项目必须使用服务端认证、密码哈希、安全的数据库存储和后台权限校验。
